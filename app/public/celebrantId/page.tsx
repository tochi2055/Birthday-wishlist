...
export default function PublicWishlist() {
  ...
  const celebrantId = params.celebrantId as string;
  ...
  useEffect(() => {
    const loadPublicWishlist = async () => {
      try {
        const response = await fetch(`/api/public-wishlist/${celebrantId}`);
        if (response.ok) {
          const data = await response.json();
          setWishlistItems(data.wishlistItems || []);
          setCelebrantSettings(data.celebrantSettings || celebrantSettings);
        } else {
          toast.error("Failed to load wishlist. Please check the link.");
        }
      } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        setWishlistItems((prev) =>
          prev.map((item) =>
            selectedItems.includes(item.id)
              ? { ...item, reserved: (item.reserved || 0) + 1 }
              : item
          )
        );
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        {wishlistItems.length === 0 ? (
          <div className="text-center py-12">
            <Card className="bg-white/95 backdrop-blur-sm dark:bg-gray-900/95 max-w-md mx-auto">
              <CardContent className="p-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  The wishlist is being prepared. Please check back soon!
                </p>
              </CardContent>
            </Card>
          </div>
